...
using ServerReaderWriter =
    grpc::ServerReaderWriter<p4::bm::PacketStreamResponse,
                             p4::bm::PacketStreamRequest>;
...
class DataplaneInterfaceServiceImpl
 ...
 private:
  ...
  Status PacketStream(ServerContext *context,
    ...
    {
      Lock lock(mutex);
      if (!started) return Status(StatusCode::UNAVAILABLE, "not ready");
      if (active) {
  ...
  Status SetPortOperStatus(
      ServerContext *context, const p4::bm::SetPortOperStatusRequest *request,
...
class NotificationsCapture : public bm::TransportIface {
 public:
  explicit NotificationsCapture(bm::SwitchWContexts *sw) : sw(sw) {}
 ...
 private:
  ...
  int send_generic(const std::string &msg) const {
    ...
    if (!memcmp("SWP|", msg.data(), 4)) {
      ...
      handle_LEA(reinterpret_cast<const LEA_hdr_t *>(&storage), data,
                 msg.size() - hdr_size);
    } else if (!memcmp("AGE|", msg.data(), 4)) {
      handle_AGE(reinterpret_cast<const AGE_hdr_t *>(&storage), data,
                 msg.size() - hdr_size);
    }
    return 0;
  }
  ...
  int open_() override { return 0; }

  int send_(const std::string &msg) const override { return send_generic(msg); }
...
SimpleSwitchGrpcRunner::SimpleSwitchGrpcRunner(
    bool enable_swap, std::string grpc_server_addr,
    bm::DevMgrIface::port_t cpu_port, std::string dp_grpc_server_addr,
    bm::DevMgrIface::port_t drop_port, std::shared_ptr<SSLOptions> ssl_options,
    size_t nb_queues_per_port)
    : simple_switch(
          new SimpleSwitch(enable_swap, drop_port, nb_queues_per_port)),
...
int SimpleSwitchGrpcRunner::init_and_start(const bm::OptionsParser &parser) {
...
#ifdef WITH_SYSREPO
  sysrepo_driver = std::unique_ptr<SysrepoDriver>(
      new SysrepoDriver(parser.device_id, simple_switch.get()));
#endif  // WITH_SYSREPO
  ...
  auto my_transport =
      std::make_shared<NotificationsCapture>(simple_switch.get());
  ...
  auto transmit_fn = [this](bm::DevMgrIface::port_t port_num,
    ...
    if (cpu_port > 0 && port_num == cpu_port) {
      ...
      auto status = pi_packetin_receive(simple_switch->get_device_id(), buf,
  ...
  if (ssl_options != nullptr) {
    ...
    if (ssl_options->with_client_auth) {
      ...
      pi_ssl_options.client_auth = PI_GRPC_SSL_DONT_REQUEST_CLIENT_CERTIFICATE;
    }
  }
  PIGrpcServerRunV2(grpc_server_addr.c_str(), nullptr,
  ...
  // TM: this is a hack to give the actions to the TM (while no compiler work is
  // done)
  simple_switch->init_tm();

  return 0;
}

void SimpleSwitchGrpcRunner::wait() { PIGrpcServerWait(); }

void SimpleSwitchGrpcRunner::shutdown() {
  if (!dp_grpc_server_addr.empty()) dp_grpc_server->Shutdown();
  PIGrpcServerShutdown();
}

void SimpleSwitchGrpcRunner::block_until_all_packets_processed() {
  simple_switch->block_until_no_more_packets();
}

bool SimpleSwitchGrpcRunner::is_dp_service_active() {
...
SimpleSwitchGrpcRunner::~SimpleSwitchGrpcRunner() { PIGrpcServerCleanup(); }

void SimpleSwitchGrpcRunner::port_status_cb(
    bm::DevMgrIface::port_t port,
