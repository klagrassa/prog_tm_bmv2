...
struct hash_ex {
  uint32_t operator()(const char *buf, size_t s) const {
    ...
    for (size_t i = 0; i < s; i++) hash = (hash ^ buf[i]) * p;
...
class SimpleSwitch::InputBuffer {
 ...
 private:
  ...
  int push_front(QueueImpl *queue, size_t capacity,
                 std::condition_variable *cvar, std::unique_ptr<Packet> &&item,
                 bool blocking) {
    Lock lock(mutex);
...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port,
    ...
    : Switch(enable_swap),
      drop_port(drop_port),
      input_buffer(new InputBuffer(1024 /* normal capacity */,
                                   1024 /* resubmit/recirc capacity */)),
      nb_queues_per_port(nb_queues_per_port),
      egress_buffers(nb_egress_threads, 64,
  ...
  // Create a TrafficManager instance
  // this->traffic_manager =
  //     std::unique_ptr<bm::TrafficManager>(new bm::TrafficManager());
  this->traffic_manager = std::unique_ptr<bm::TrafficManager>(
      new bm::TrafficManager(&egress_buffers));

  import_primitives(this);
}

/**
 * @brief Initialize the Traffic Manager.
 * This method must be called AFTER the switch is fully inited, ie P4 program
 * fully parsed.
 */
void SimpleSwitch::init_tm() {
  BMLOG_DEBUG("Trying to get P4Objects instance");
  bm::Context *cxt = get_context(0);
  if (cxt) {
    std::shared_ptr<bm::P4Objects> p4objects = cxt->get_p4objects();
    if (p4objects) {
      BMLOG_DEBUG("Got P4Objects instance");

      // Loop around scheduler specific functions
      // TODO(klagrassa) Find a way to not hardcode the scheduler types.

      // First select type between a selection of scheduler
      std::vector<std::string> scheduler_types = {"FIFO", "DRR", "WRR", "SP"};

      // Then, create the action_names
      std::vector<std::string> action_names = {"calculate_rank",
                                               "evaluate_predicate", "dequeued",
                                               "periodic_timeout"};

      // Finally, concatenate the scheduler types with the actions names
      std::vector<std::string> action_entries_name;
      for (const auto &scheduler : scheduler_types) {
        for (const auto &action : action_names) {
          action_entries_name.push_back(scheduler + "_" + action);
        }
      }

      // Loop around the action_entries_name to check the action names
      for (const auto &action_entry_name : action_entries_name) {
        bm::ActionFn *action_fn = p4objects->get_one_action_with_name(
            "MyIngress." + action_entry_name);
        if (action_fn) {
          std::cout << "Action function found: " << action_fn->get_name()
                    << std::endl;
          traffic_manager->add_action(action_entry_name, action_fn);
        }
      }
      traffic_manager->set_actions();
    } else {
      BMLOG_DEBUG("Failed to get P4Objects instance");
    }
  } else {
    BMLOG_DEBUG("Failed to get switch context");
  }
}
...
uint64_t SimpleSwitch::get_time_elapsed_us() const { return get_ts().count(); }

uint64_t SimpleSwitch::get_time_since_epoch_us() const {
  auto tp = clock::now();
...
void SimpleSwitch::enqueue(port_t egress_port,
                           std::unique_ptr<Packet> &&packet) {
  ...
  size_t priority =
      phv->has_field(SSWITCH_PRIORITY_QUEUEING_SRC)
  ...
  // TrafficManager addition
  this->traffic_manager->enqueue(egress_port, std::move(packet));

  /* Poll the traffic manager */
  /* Implement a waiting condition is better IMO */
  // std::unique_ptr<Packet> pkt_out_tmp =
  //     std::move(this->traffic_manager->dequeue());

  // if (pkt_out_tmp) {
  //   // egress_buffers.push_front(egress_port, priority, std::move(packet));
  //   egress_buffers.push_front(pkt_out_tmp->get_egress_port(),
  //                             std::move(pkt_out_tmp));
  // }
}
...
void SimpleSwitch::copy_field_list_and_set_type(
    ...
    const std::unique_ptr<Packet> &packet_copy, PktInstanceType copy_type,
...
void SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    // packet = std::move(this->traffic_manager->dequeue());

    if (packet == nullptr) break;
    port = packet->get_egress_port();
    ...
    if (with_queueing_metadata) {
      ...
      phv->get_field("queueing_metadata.deq_timedelta")
          .set(get_ts().count() - enq_timestamp);
      // phv->get_field("queueing_metadata.deq_qdepth")
      //     .set(egress_buffers.size(port, priority));
    ...
    phv->get_field("standard_metadata.packet_length")
        .set(packet->get_register(RegisterAccess::PACKET_LENGTH_REG_IDX));
