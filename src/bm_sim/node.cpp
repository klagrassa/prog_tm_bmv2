...
bm::Node::Node(int id) {
  ...
  // enqueue_callback = [this]() {
  //   std::thread([this]() { this->eval_predicate(); }).detach();
  // };
...
void bm::Node::run() {
  while (true) {
    ...
    if (!task_queue.empty()) {
      ...
      // No new tasks, but packets are still in the store
      // if (!calendar_store.empty() &&
      //     (predicate_set && predicate_rank.second != 0)) {
...
#ifdef BM_ENABLE_TM_DEBUG
  BMLOG_DEBUG("Dequeuing packet from the Node - {}, {} given", pred_to_dq.first,
              pred_to_dq.second);
#endif
...
#endif
  //   std::unique_lock<std::mutex> lock(eval_predicate_mutex,
  //   std::try_to_lock); if (!lock.owns_lock()) {
  // #ifdef BM_ENABLE_TM_DEBUG
  //     BMLOG_DEBUG("Failed to acquire the lock");
  // #endif
  //     return;
  //   }
  // #ifdef BM_ENABLE_TM_DEBUG
  //   BMLOG_DEBUG("Passed the lock");
  // #endif
