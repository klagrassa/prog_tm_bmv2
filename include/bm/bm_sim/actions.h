...
#define REGISTER_PRIMITIVE_W_NAME(primitive_name, primitive)                 \
  bool primitive##_create_ =                                                 \
      bm::ActionOpcodesMap::get_instance()->register_primitive(              \
          primitive_name, []() {                                             \
            return std::unique_ptr<::bm::ActionPrimitive_>(new primitive()); \
          })

struct ActionData {
  const Data &get(int offset) const { return action_data[offset]; }

  void push_back_action_data(const Data &data) { action_data.push_back(data); }
...
struct ActionParam {
  ...
  enum {
    ...
    EXPRESSION,
    EXPRESSION_HEADER,
    EXPRESSION_HEADER_STACK,
    EXPRESSION_HEADER_UNION,
    EXPRESSION_HEADER_UNION_STACK,
    EXTERN_INSTANCE,
    STRING,
    HEADER_UNION,
    HEADER_UNION_STACK,
  ...
  template <typename T>
  T to(ActionEngineState *state) const;
};
...
struct ActionEngineState {
  ...
  ActionEngineState(Packet *pkt, const ActionData &action_data,
                    const std::vector<Data> &const_values,
                    const std::vector<ActionParam> &parameters_vector,
                    const std::vector<ActionParam> &field_list);
};
...
template <>
inline Data &ActionParam::to<Data &>(ActionEngineState *state) const {
  ...
  switch (tag) {
    ...
    case ActionParam::LAST_HEADER_STACK_FIELD:
      return state->phv.get_header_stack(stack_field.header_stack)
          .get_last()
...
template <>
inline const Data &ActionParam::to<const Data &>(
...
template <>
inline Field &ActionParam::to<Field &>(ActionEngineState *state) const {
  assert(tag == ActionParam::FIELD);
  return state->phv.get_field(field.header, field.field_offset);
}

template <>
inline const Field &ActionParam::to<const Field &>(
    ActionEngineState *state) const {
  return ActionParam::to<Field &>(state);
}

template <>
inline Header &ActionParam::to<Header &>(ActionEngineState *state) const {
  switch (tag) {
...
template <>
inline const Header &ActionParam::to<const Header &>(
    ActionEngineState *state) const {
  return ActionParam::to<Header &>(state);
}

template <>
inline HeaderStack &ActionParam::to<HeaderStack &>(
    ActionEngineState *state) const {
  switch (tag) {
...
template <>
inline const HeaderStack &ActionParam::to<const HeaderStack &>(
    ActionEngineState *state) const {
  return ActionParam::to<HeaderStack &>(state);
}

template <>
inline StackIface &ActionParam::to<StackIface &>(
    ActionEngineState *state) const {
  switch (tag) {
    case HEADER_STACK:
      return state->phv.get_header_stack(header_stack);
...
template <>
inline const StackIface &ActionParam::to<const StackIface &>(
    ActionEngineState *state) const {
  return ActionParam::to<StackIface &>(state);
}

template <>
inline HeaderUnion &ActionParam::to<HeaderUnion &>(
    ActionEngineState *state) const {
  switch (tag) {
...
template <>
inline const HeaderUnion &ActionParam::to<const HeaderUnion &>(
    ActionEngineState *state) const {
  return ActionParam::to<HeaderUnion &>(state);
}

template <>
inline HeaderUnionStack &ActionParam::to<HeaderUnionStack &>(
    ActionEngineState *state) const {
  switch (tag) {
...
template <>
inline const HeaderUnionStack &ActionParam::to<const HeaderUnionStack &>(
    ActionEngineState *state) const {
  return ActionParam::to<HeaderUnionStack &>(state);
}

template <>
inline const NamedCalculation &ActionParam::to<const NamedCalculation &>(
    ActionEngineState *state) const {
  (void)state;
...
template <>
inline MeterArray &ActionParam::to<MeterArray &>(
    ActionEngineState *state) const {
  (void)state;
...
template <>
inline const MeterArray &ActionParam::to<const MeterArray &>(
    ActionEngineState *state) const {
  return ActionParam::to<MeterArray &>(state);
}

template <>
inline CounterArray &ActionParam::to<CounterArray &>(
    ActionEngineState *state) const {
  (void)state;
...
template <>
inline const CounterArray &ActionParam::to<const CounterArray &>(
    ActionEngineState *state) const {
  return ActionParam::to<CounterArray &>(state);
}

template <>
inline RegisterArray &ActionParam::to<RegisterArray &>(
    ActionEngineState *state) const {
  (void)state;
...
template <>
inline const RegisterArray &ActionParam::to<const RegisterArray &>(
    ActionEngineState *state) const {
  return ActionParam::to<RegisterArray &>(state);
}

template <>
inline ExternType *ActionParam::to<ExternType *>(
    ActionEngineState *state) const {
  (void)state;
...
template <>
inline const std::string &ActionParam::to<const std::string &>(
    ActionEngineState *state) const {
...
// one
template <>
inline const char *ActionParam::to<const char *>(
    ActionEngineState *state) const {
  assert(tag == ActionParam::STRING);
  (void)state;
  return str->c_str();
...
template <>
inline const std::vector<Data> ActionParam::to<const std::vector<Data>>(
    ActionEngineState *state) const {
...
template <>
inline const std::vector<Field> ActionParam::to<const std::vector<Field>>(
    ActionEngineState *state) const {
...
struct unpack_caller {
 private:
  ...
  void call(T *pObj, ActionEngineState *state, const ActionParam *args,
            const indices<I...>) {
...
#undef ELEM_TYPE
 ...
 public:
  template <typename T>
  void operator()(T *pObj, ActionEngineState *state, const ActionParam *args) {
...
class ActionPrimitive_ {
 public:
  ...
  virtual void execute(ActionEngineState *state, const ActionParam *args) = 0;
  ...
  void _set_p4objects(P4Objects *p4objects) { this->p4objects = p4objects; }
 ...
 protected:
  ...
  P4Objects *get_p4objects() { return p4objects; }
...
class ActionPrimitive : public ActionPrimitive_ {
 ...
 protected:
  ...
  Field &get_field(const std::string &name) { return phv->get_field(name); }

  //! Returns a reference to the Header with name \p name.
  Header &get_header(const std::string &name) { return phv->get_header(name); }
...
class ActionFn : public NamedP4Object {
 ...
 public:
  ...
  // TM proto add
  void update_extern_instance(ExternType *new_extern);
...
class ActionFnEntry {
 public:
  ...
  explicit ActionFnEntry(const ActionFn *action_fn) : action_fn(action_fn) {}
  ...
  const ActionData &get_action_data() const { return action_data; }
  ...
  const ActionFn *get_action_fn() const { return action_fn; }
