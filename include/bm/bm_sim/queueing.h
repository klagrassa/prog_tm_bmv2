...
class QueueingLogic {
 ...
 public:
  ...
  void push_front(size_t queue_id, const T &item) {
    ...
    element_count++; // Prototype TM : general packet count
  }
  ...
  //! Get the occupancy of all logical queues.
  //! Prototype TM : general packet count
  bool empty() const {
    LockType lock(mutex);
  ...
  /**
   * @brief Converts the queueing logic information to a string representation.
   *
   * This function generates a string that represents the current state of the
   * queueing logic. The output string includes the header "QueueingLogic" followed
   * by information about each queue. For each queue, the string contains:
   * - The queue identifier (first element of the pair in `queues_info`).
   * - The current size of the queue (second element's `size` in `queues_info`).
   * - The capacity of the queue (second element's `capacity` in `queues_info`).
   *
   * @return A string representing the current state of the queueing logic.
   */
  std::string to_string() const {
    LockType lock(mutex);
    std::string out = "QueueingLogic\n";
    for (const auto &p : queues_info) {
      out += std::to_string(p.first) + " : " + std::to_string(p.second.size) +
             " / " + std::to_string(p.second.capacity) + "\n";
    }
    return out;
  }
 ...
 private:
  struct QE {
    QE(T e, size_t queue_id) : e(std::move(e)), queue_id(queue_id) {}
  ...
  struct QueueInfo {
    explicit QueueInfo(size_t capacity) : capacity(capacity) {}
  ...
  QueueInfo &get_queue(size_t queue_id) {
    ...
    // member)
    auto p = queues_info.emplace(std::piecewise_construct,
  ...
  int element_count = 0; // Prototype TM : general packet count
...
class QueueingLogicPriRL {
 ...
 public:
  ...
  QueueingLogicPriRL(size_t nb_workers, size_t capacity, FMap map_to_worker,
 ...
 private:
  ...
  struct QueueInfo : public std::vector<QueueInfoPri> {
    QueueInfo(size_t capacity, uint64_t queue_rate_pps, size_t nb_priorities)
        : std::vector<QueueInfoPri>(nb_priorities,
                                    QueueInfoPri(capacity, queue_rate_pps)) {}
